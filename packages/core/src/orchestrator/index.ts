 ...
 * Orchestrator
 *
 * Discovers services, builds dependency graph, builds images, runs deploy.ts files.
...
/**
 * K3d configuration from deploy.yaml
 */
interface K3dConfig {
...
/**
 * Root configuration from deploy.yaml
 */
...
/**
 * Discover services from services/ directory
 */
export const discoverServices = (rootPath: string): Either<DeployError, readonly DiscoveredService[]> => {
  ...
  try {
    ...
    for (const name of entries) {
      ...
      const deployPath = path.join(servicePath, 'deploy.ts')
      const configPath = path.join(servicePath, 'deploy.yaml')
      ...
      const dependencies = (rawConfig.dependencies as string[]) ?? []
...
export class Orchestrator {
  ...
  deploy(config: OrchestratorConfig): EitherAsync<DeployError, OrchestratorResult> {
    ...
    return EitherAsync(async ({ liftEither, throwE }) => {
      ...
      const rootConfigPath = path.join(config.rootPath, 'deploy.yaml')
      ...
      const allServices = await liftEither(discoverServices(config.rootPath))
      const services = filterServices(allServices, config.servicesToDeploy)

      for (const service of services) {
        this.eventEmitter.emitLog('info', service.name, undefined, 'Discovery')
      }
